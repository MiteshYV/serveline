...
export function BatchForm() {
  ...
  return (
    <form action={action} className={dash.form}>
      ...
      {state.error && <p className={dash.error} role="status">{state.error}</p>}
      {state.created && <p className={dash.hint} role="status">{state.created} cards generated. Print them from the list below.</p>}
