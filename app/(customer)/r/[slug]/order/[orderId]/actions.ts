...
import { attachPayment, getOrder, withdrawConsent } from '@/db/repos/index.ts'
 ...
 * the older awaiting rows is attachPayment's concern, not this action's.
 */
export async function reissuePaymentLinkAction(formData: FormData): Promise<void> {
  ...
  await attachPayment(
    ...
    { type: 'customer', id: session.subjectId },
  )
